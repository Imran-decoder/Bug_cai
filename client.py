...
#     token: str = Field(description="A GitHub Personal Access Token.", default=os.getenv("GITHUB_ACCESS_TOKEN", ""))
#
#
...
#         description="The optional name for the new local directory. If not provided, the repository name is used.",
#         default=None)
#
#
...
#             ("system",
#              "You are a helpful assistant that manages a data science workflow. You have access to filesystem tools, data querying tools, and GitHub integration tools. If a git push fails because the remote has diverged, you should use the git pull tool first to merge the changes before trying to push again."),
#             ("human", "{query}"),
...
#             "gh_push": GhPushArgs,
#             "gh_pull": GhPullArgs,
#             "gh_clone": GhCloneArgs,
#         }
#         langchain_tools = []
#         for tool_name, args_schema in tool_to_args_map.items():
#             new_tool = MCPTool(
#                 name=tool_name,
#                 description=args_schema.__doc__ or f"Tool: {tool_name}",
#                 args_schema=args_schema,
#                 # remove session if it’s optional OR
